...
export function presidentialReaction(policyAction: number): string {
  if (policyAction <= -50) return '“Finally. Very strong. Could have been stronger.”';
  if (policyAction < 0) return '“A cut. Good. Small, but good. I know people who would cut more.”';
  if (policyAction === 0) return '“UNCHANGED? Interesting strategy. Very academic.”';
  if (policyAction <= 25) return '“Rates UP? Fantastic, if your goal is making everything expensive.”';
  return '“Fifty basis points higher. The Chair has declared war on brunch.”';
}
